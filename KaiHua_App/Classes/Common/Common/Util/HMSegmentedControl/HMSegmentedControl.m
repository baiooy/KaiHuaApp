...
- (void)setDefaults {
    ...
    self.selectionIndicatorColor = [UIColor whiteColor];
...
- (void)drawRect:(CGRect)rect
{
   ...
   // [self.textColor set];
    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        UIImage *img = [UIImage imageNamed:titleString];
        UIButton *btnImg = [UIButton buttonWithType:UIButtonTypeCustom];
        [btnImg setImage:img forState:UIControlStateNormal];
        //[btnImg setFrame:CGRectMake(0, 0, _segmentWidth, img.size.height/2)];
        [btnImg setUserInteractionEnabled:NO];
        [btnImg setBackgroundColor:[UIColor clearColor]];
        
        CGFloat y = ((self.height - self.selectionIndicatorHeight) / 2) + (self.selectionIndicatorHeight - _segmentWidth / 2);
        CGRect rect = CGRectMake(self.segmentWidth * idx, y, self.segmentWidth, _segmentWidth);
        [btnImg setFrame:rect];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        ...
        return CGRectMake(self.segmentWidth * self.selectedIndex, self.frame.size.height, self.segmentWidth, self.selectionIndicatorHeight);
    }
}
...
- (void)updataIndicator:(NSUInteger)index animated:(BOOL)animated{
