...
@protocol HLoadingView
- (void)hideWithAnimate;
@end

static HLoadingView *sShareLoadingView;
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self = [super initWithFrame:frame]) {
		...
		UIViewAutoresizing viewAutoResizing = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleBottomMargin;
		...
		imageView = [[UIImageView alloc] initWithFrame:CGRectMake(boardView.bounds.size.width/2 - 51, 15, 102, 102)];
...
{
	sShareLoadingView = [HLoadingView shareInstance];
...
+ (void)showImage:(UIImage *)image
         ...
         autoHide:(BOOL)autoHide
{
	[HLoadingView showInView:MainView
                      image:image
					  info:info
                   animated:NO
                    modeled:NO
...
+ (void)showLoadingViewHintMessage:(NSString *)message{
    ...
    [HLoadingView showInView:MainView
                       image:nil
                        info:message
                    animated:YES modeled:YES autoHide:YES];
}
